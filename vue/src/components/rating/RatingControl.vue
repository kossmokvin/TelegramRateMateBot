<template>
  <div class="rating-control-component">
...
import RatingControlPrototype from "@/components/rating/prototype/RatingControlPrototype";
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isDarkScheme = WebApp?.colorScheme === "dark";
    const themeParams = WebApp?.themeParams.bg_color
      ? WebApp?.themeParams
      : {
          bg_color: "#1d2733",
    ...
    const faceColor = isDarkScheme
      ? themeParams.hint_color
      : themeParams.secondary_bg_color;
    const faceDetailsColor = isDarkScheme
    ...
    const init = () => {
      var RatingControl = function (element) {
        ...
        self.selectedRatingElement = self.containerElement?.querySelector(
          ".current-rating"
        );
        self.selectedRatingSVGContainer = self.selectedRatingElement?.querySelector(
          ".svg-wrapper"
        );
        self.ratingElements = [].slice
          .call(self.containerElement?.querySelectorAll(".rating-option"))
          .map(function (element) {
            return {
              container: element,
              icon: element.querySelector(".icon"),
              label: element.querySelector(".label"),
              selectedFill: self.hexToRGB(
                element.getAttribute("selected-fill") || "#FFD885"
              ),
            };
          });
        ...
        self.facePaths = [];
        self.labelColor = self.hexToRGB(themeParams.hint_color || "#7d8b99");
        self.labelSelectedColor = self.hexToRGB(
        ...
        self.easings = {
          easeInOutCubic: function (t, b, c, d) {
            const low = (t /= d / 2) < 1;
            if (low) return (c / 2) * t * t * t + b;
            return (c / 2) * ((t -= 2) * t * t + 2) + b;
          },
          easeInOutQuad: function (t, b, c, d) {
            const low = (t /= d / 2) < 1;
            if (low) return (c / 2) * t * t + b;
        ...
        self.ratingElements.forEach(function (element) {
          ...
          var paths = {};
          [].forEach.call(
            element.icon.querySelectorAll("path:not(.base)"),
            function (path) {
          ...
          element.container.addEventListener(
            "ontouchend" in document ? "touchend" : "click",
            function (e) {
              if ("ontouchend" in document) {
                var ratingTouchCurrentPosition = { x: e.pageX, y: e.pageY };
                var dragDistance = Math.sqrt(
                  Math.pow(
                    ratingTouchCurrentPosition.x -
                      self.ratingTouchStartPosition.x,
                    2
                  ) +
                    Math.pow(
                      ratingTouchCurrentPosition.y -
                        self.ratingTouchStartPosition.y,
                      2
                    )
                );
                if (dragDistance > 10) {
              ...
              emit("update:modelValue", newRating + 1);
        ...
        if ("ontouchend" in document) {
          ...
          self.selectedRatingElement.addEventListener("touchstart", function (
            e
          ) {
            self.dragging = true;
            self.handleDragOffset =
              e.touches[0].pageX -
              self.selectedRatingElement.getBoundingClientRect().left;
            self.setLabelTransitionEnabled(false);
          });
          self.selectedRatingElement.addEventListener(
            "touchmove",
            self.onHandleDrag
          );
          self.selectedRatingElement.addEventListener(
            "touchend",
            self.onHandleRelease
          );
        } else {
        ...
        self.setRating(props.modelValue - 1, { duration: 0, silent: 1 });
      };

      RatingControl.prototype = RatingControlPrototype;

      document.querySelector(".rating-container")?.classList.add("clip-marker");
      new RatingControl(document.querySelector(".rating-control"));
    };

    return { faceColor, faceDetailsColor, colorScheme };
  },
};
</script>
...
.rating-control .current-rating {
  ...
  will-change: transform;
  cursor: -webkit-grab;
  cursor: grab opacity 0 transition opacity 0.4875s
